...
RUN set -x \
    ...
    && curl -Ls                "https://www.atlassian.com/software/jira/downloads/binary/atlassian-servicedesk-${SERVICEDESK_VERSION}.tar.gz" | tar -xz --directory "${JIRA_INSTALL}" --strip-components=1 --no-same-owner \
...
# Copy all custom plugins to the installation directory
COPY plugins ${JIRA_HOME}/plugins/installed-plugins
